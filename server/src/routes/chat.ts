...
import { authOptional, type AuthedRequest } from "../middleware/auth.js";
import crypto from "crypto";

const router = Router();

// 오프라인 모의 지식베이스 (3개 국어 지원 및 일상 대화 확장)
...
const KB_RESOURCES: Record<string, KnowledgeBase> = {
  ko: {
...
function analyzeMessageOffline(message: string, locale: string): string {
...
// 챗 히스토리 조회 API
router.get("/history", authOptional, async (req: AuthedRequest, res: Response): Promise<void> => {
  try {
    ...
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT id, userId, sender, text, locale, createdAt FROM chatmessage WHERE userId = ? ORDER BY createdAt DESC LIMIT 30`,
      userId
    );
...
router.post("/", authOptional, async (req: AuthedRequest, res: Response): Promise<void> => {
  try {
    ...
    // 대화 메시지 저장 헬퍼
    const saveMessageToDb = async (sender: "user" | "bot", text: string) => {
      const msgId = crypto.randomUUID();
      try {
        await prisma.$executeRawUnsafe(
          `INSERT INTO chatmessage (id, userId, sender, text, locale, createdAt) VALUES (?, ?, ?, ?, ?, ?)`,
          msgId,
          userId,
          sender,
          text,
