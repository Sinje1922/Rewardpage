 ...
 * 캠페인 생성 전 보유 토큰 확인용
 */
router.get("/balance/:address", authRequired, async (req: AuthedRequest, res) => {
 ...
 * 특정 토큰 잔액만 조회
 */
router.get("/balance/:address/:currency", authRequired, async (req: AuthedRequest, res) => {
  const address = req.params.address as string;
  ...
  try {
    const result = await getTokenBalance(address, currency.toUpperCase() as "USDT" | "METAQ");
    res.json({
      address,
 ...
 */
router.get("/escrow/:campaignId", authRequired, async (req: AuthedRequest, res) => {
