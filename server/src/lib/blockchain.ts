...
type NetworkKey = keyof typeof BSC_CONFIG;
 ...
 * 특정 토큰의 컨트랙트 주소 반환
 */
export function getTokenAddress(currency: "USDT" | "METAQ"): string {
  const config = getConfig();
  if (currency === "USDT") return config.USDT_ADDRESS;
  if (currency === "METAQ") return config.METAQ_ADDRESS;
 ...
 */
export function campaignIdToBytes32(campaignId: string): string {
 ...
 * 여러 토큰 잔액 일괄 조회
 */
export async function getWalletTokenBalances(walletAddress: string): Promise<{
  USDT: string;
  METAQ: string;
}> {
  try {
 ...
 */
export async function getEscrowCampaignState(campaignId: string): Promise<{
  creator: string;
  token: string;
  totalAmount: string;
  lockedAt: number;
  state: number; // 0=NONE, 1=LOCKED, 2=DISTRIBUTED, 3=REFUNDED
} | null> {
  try {
    const address = process.env.ESCROW_CONTRACT_ADDRESS;
    ...
    if (result.state === 0n) return null; // NONE = 미등록
 ...
 */
export async function distributeOnChainRewards(
  campaignId: string,
  currency: "USDT" | "METAQ",
  ...
  const winnerAmounts = winners.map((w) =>
    ethers.parseUnits(w.amount.toFixed(18), DECIMALS)
  );

  try {
    const tx = await escrow.distributeRewards(
      campaignKey,
      winnerAddresses,
      winnerAmounts,
      { gasLimit: 500000 }
 ...
 */
export async function refundCreatorOnChain(campaignId: string): Promise<{ txHash: string }> {
