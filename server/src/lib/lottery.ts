export function pickWeightedUnique(userIds: string[], weights: number[], count: number): string[] {
  const entries: string[] = [];
  for (let i = 0; i < userIds.length; i++) {
    const w = Math.max(0, Math.floor(weights[i] ?? 0));
    for (let j = 0; j < w; j++) entries.push(userIds[i]);
  }
...
export function pickUniformUnique(userIds: string[], count: number): string[] {
