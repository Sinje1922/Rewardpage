    ...
    if (c.rewardDistMode === "SEPARATE") {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            for (const [userId, wonList] of winnerRewardsMap) {
                ...
                if (totalPoints > 0)
                    userUpdateData.pointBalance = { increment: totalPoints };
                if (totalCoupon > 0)
                    userUpdateData.couponBalance = { increment: totalCoupon };
                if (totalUsdt > 0)
                    userUpdateData.usdtBalance = { increment: totalUsdt };
                if (totalBrl > 0)
                    userUpdateData.brlBalance = { increment: totalBrl };
                if (totalMetaq > 0)
                    userUpdateData.metaqBalance = { increment: totalMetaq };
        ...
        await prisma.$transaction(async (tx) => {
            ...
            for (let i = 0; i < picked.length; i++) {
                ...
                if (totalPointsPerWinner > 0)
                    userUpdateData.pointBalance = { increment: totalPointsPerWinner };
                if (totalCouponPerWinner > 0)
                    userUpdateData.couponBalance = { increment: totalCouponPerWinner };
                if (totalUsdtPerWinner > 0)
                    userUpdateData.usdtBalance = { increment: totalUsdtPerWinner };
                if (totalBrlPerWinner > 0)
                    userUpdateData.brlBalance = { increment: totalBrlPerWinner };
                if (totalMetaqPerWinner > 0)
                    userUpdateData.metaqBalance = { increment: totalMetaqPerWinner };
                if (Object.keys(userUpdateData).length > 0) {
                    await tx.user.update({
                        where: { id: userId },
                        data: userUpdateData,
 ...
 */
async function dispatchOnChainDistribution(campaignId) {
    try {
        // 당첨자 목록 조회 (지갑 주소 포함)
