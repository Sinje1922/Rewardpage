...
export async function evaluateMission(userId, missionId, type, config, payload) {
    switch (type) {
        case "LINK_VISIT": {
        ...
        case "SURVEY": {
            ...
            if (qs.length === 0) {
                ...
                if (question && !expected) {
                    if (!code)
                        return { ok: false, reason: "답변을 입력해 주세요." };
                    return { ok: true };
                }
                if (expected) {
        ...
        case "FILE_UPLOAD": {
            ...
            if (!url)
                return { ok: false, reason: "파일 URL이 필요합니다." };
            return { ok: true };
        }
        case "YOUTUBE_WATCH": {
            const { prisma } = await import("./prisma.js");
        ...
        case "DISCORD_JOIN": {
            ...
            if (!user?.discordId)
                return { ok: false, reason: "디스코드 연동이 필요합니다." };
