...
const router = Router();
function calculateRequiredBalances(rewardsConfig) {
    const reqs = {
        POINT: 0,
        USDT: 0,
        BRL: 0,
...
async function replaceCampaignMissions(campaignId, missions, bypassSubmissionsCheck = false) {
...
router.post("/", authRequired, requireRoles("MANAGER", "ADMIN"), async (req, res) => {
    ...
    const hasTokenReward = rewardsConfig.some((r) => ["USDT", "METAQ"].includes(r.currency));
    if (hasTokenReward && !creator.walletAddress) {
        res.status(400).json({ error: "토큰 보상(USDT, METAQ)을 설정하려면 메타마스크 지갑 연동이 필수입니다." });
        return;
    }
    if (creator.pointBalance < required.POINT ||
...
router.patch("/:id", authRequired, async (req, res) => {
    ...
    const hasTokenReward = finalRewardsConfig.some((r) => ["USDT", "METAQ"].includes(r.currency));
    if (hasTokenReward) {
        const creator = await prisma.user.findUnique({
    ...
    if (b.rewardsConfig !== undefined) {
        ...
        if (diff.POINT > 0 || diff.USDT > 0 || diff.BRL > 0 || diff.METAQ > 0 || diff.COUPON > 0) {
            ...
            if ((diff.POINT > 0 && creator.pointBalance < diff.POINT) ||
